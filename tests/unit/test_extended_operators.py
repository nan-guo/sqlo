...
def test_extended_operators():
    ...
    sql, params = q.build()
    assert sql == "SELECT * FROM `users` WHERE `name` NOT LIKE ?"
    assert params == ("%john%",)

    # REGEXP
    q = Q.select().from_("users").where("name REGEXP", "^J.*")
    sql, params = q.build()
    assert sql == "SELECT * FROM `users` WHERE `name` REGEXP ?"
    assert params == ("^J.*",)

    # Custom/Weird Operator (just to prove regex flexibility)
    q = Q.select().from_("users").where("age MY_OP", 10)
    sql, params = q.build()
    assert sql == "SELECT * FROM `users` WHERE `age` MY_OP ?"
    assert params == (10,)
...
def test_is_operator():
    ...
    sql, params = q.build()
    assert sql == "SELECT * FROM `users` WHERE `active` IS ?"
    assert params == (True,)
